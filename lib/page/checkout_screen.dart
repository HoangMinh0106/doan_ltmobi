import 'dart:math'; // **SỬA LỖI**: Thêm import này
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi khi lấy vị trí: $e')));
      }
    } finally {
      if (mounted) {
  ...
  // **SỬA LỖI**: Đảm bảo tổng tiền không bị âm
  double get _finalPrice => max(0, widget.totalPrice - _discountAmount - _membershipDiscount);
  ...
  Future<void> _processOrderCreation() async {
    ...
    try {
      ...
      // **SỬA LỖI**: Vô hiệu hóa voucher sau khi sử dụng
      if (_appliedVoucher != null) {
        await MongoDatabase.invalidateVoucher(_appliedVoucher!['_id']);
      }
  ...
  void _payWithPayPal() {
    if (_finalPrice <= 0 && widget.totalPrice > 0) { // Nếu tổng tiền là 0 nhưng không phải giỏ hàng rỗng
      _processOrderCreation(); // Coi như đơn hàng 0 đồng, xử lý như COD
      return;
    }
    if (_finalPrice <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Không thể thanh toán cho đơn hàng 0 VNĐ.')),
      );
      return;
    }
        ...
        child: AnimatedContainer(
          ...
          child: Row(
            children: [
              ...
              if (selected) const Icon(Icons.check_circle, color: primaryColor, size: 24),
            ],
          ),
        ),
      ),
